...
import { addLanguage, createRoom, openRoom, uniqueRoomId } from "./utils";

// Ported from v1 (survey-creator-collaboration/e2e/translation-sync.spec.ts)
// and adapted to the journal plugin: rooms instead of sessions, and the tab
// refresh lives in JournalTabRefresher (journal plugin).
...
test.describe("collaborative translation sync", () => {
    ...
    test("incoming remote edits don't steal focus from the cell being edited", async ({ page, context }) => {
        const roomId = uniqueRoomId("tr-focus");
        await createRoom(page, roomId, TWO_QUESTION_SCHEMA);
    ...
    test("concurrent edits to the same field in different languages merge", async ({ page, context }) => {
        const roomId = uniqueRoomId("tr-merge");
        await createRoom(page, roomId, MULTI_LOCALE_SCHEMA);
    ...
    // registers a brand-new locale as an *unchecked* row: it appears in the
    // sidebar list so the receiver can opt in, but their matrix columns stay
    // untouched. Otherwise an edit in a language you never enabled would
    // silently widen your table.
    test("a new language added by one client shows in the other's language list but not as a matrix column", async ({ page, context }) => {
        const roomId = uniqueRoomId("tr-newlang");
