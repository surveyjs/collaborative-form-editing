...
// Ported from v1 (survey-creator-collaboration/e2e/logic-sync.spec.ts) and
// adapted to the journal plugin: rooms instead of sessions, and the deferred
// Logic-tab rebuild now lives in JournalTabRefresher (journal plugin).
...
test.describe("collaborative logic sync", () => {
    ...
    // status bar eats vertical space; a narrow width collapses tabs into an
    // overflow popup).
    ...
    test("a remote rule edit refreshes the other client's rule list", async ({ page, context }) => {
        const roomId = uniqueRoomId("lg-refresh");
        await createRoom(page, roomId, ONE_RULE_SCHEMA);
        ...
        // B is in view mode: the refresher rebuilds its list, so the rule row
    ...
    // edits.
    test("a remote edit does not disturb the rule the other client is editing, and merges on save", async ({ page, context }) => {
        const roomId = uniqueRoomId("lg-defer");
        await createRoom(page, roomId, TWO_RULE_SCHEMA);
